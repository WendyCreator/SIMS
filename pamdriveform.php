...
<script>(function(d, s, id) {
  ...
  fjs.parentNode.insertBefore(js, fjs);
         ...
         <div class="container">
            <div class="row">
               <div class="col-lg-8">
                  ...
                  <?php 
                  if(isset($_POST['img-submit']) or isset($_POST['head-submit']) or isset($_POST['btn-submit']) or isset($_POST['side-head'])){
                  $blog = $conn->real_escape_string($_POST['blog_id']);
                     $blog = $conn->query("SELECT * FROM blog WHERE dblog_id='$blog'");
                     
                     while($row=$blog->fetch_assoc()):
                        $blog_title = str_replace(' ', '-', $row['dtitle']);
                        header("Location:blog/$blog_title");
                     ?>
                     ...
                     <div class="single-blog">
                         <form action="blog" method="post">
                            <input type="hidden" name="blog_id" value=<?php echo $row['dblog_id'] ?>>
                        <div class="blog-image">
                            <button type="submit" name='img-submit'>
                                <img src="_slide_blog/<?php echo $row['dimg']; ?>" alt="blog image" />
                            </button>
                            
                            </div>
                            <div class="blog-text">
                                <button type="submit" name='head-submit'>
                                    <h3><?php echo $row['dtitle']; ?></h3>
                                </button>
                                ...
                                <button type="submit" name='btn-submit'>
                                     <a class="gauto-btn" class="read-more">read more</a> 
                                </button>
                            </form>
               ...
               <div class="col-lg-4">
                  <div class="blog-page-right">
                     ...
                     <div class="sidebar-widget">
                        ...
                        <?php 
                           $blog1 = $conn->query("SELECT * FROM blog ORDER BY id DESC LIMIT 10");
                           while($row1=$blog1->fetch_assoc()):
                              $blog_title = str_replace(' ', '-', $row1['dtitle']);
                           ?>
                           <li>
                              <div class="recent-img">
                              <form action="blog" method="post">
                            <input type="hidden" name="blog_id" value=<?php echo $row1['dblog_id'] ?>>
                                 <a href="#">
                                    <img src="_slide_blog/<?php echo $row1['dimg']; ?>" alt="blog image" />
                                 </a>
                              </div>
                              <div class="recent-text">
                                  <button type="submit" name='side-head'>
                                      <h4>
                                     <?php echo $row1['dtitle']; ?>
                                      </h4>
                                  </button>
                                 <form>
                              </div>
      ...
      <script>
          ...
          const submitPost = document.querySelectorAll('.postId')
          const readMore = document.querySelectorAll('.read-more')
          submitPost.forEach(post=>{
              post.style.display = 'none';
          })
          postRefs.forEach(postRef=>{
              postRef.addEventListener('click', function(){
